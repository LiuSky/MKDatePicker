//
//  PickerViewController+DateHourMinuteSecond.swift
//  XBDatePicker
//
...
/// MARK - PickerViewController + DateHourMinuteSecond
extension PickerViewController {
    
    /// MARK - DateHourMinuteSecond
    final class DateHourMinuteSecond: NSObject {}
}


/// MARK - PickerViewControllerDataSource
extension PickerViewController.DateHourMinuteSecond: PickerViewControllerDataSource {
    ...
    public func pickerDefaultSelected(_ contentView: PickerViewController) {
        ...
        contentView.selectRow(contentView.secondIndex, inComponent: 5, animated: true)
    }
...
extension PickerViewController.DateHourMinuteSecond: PickerViewControllerDelegate {
    ...
    public func pickerContentView(_ contentView: PickerViewController, didSelectRow row: Int, inComponent component: Int) {
        ...
        case 0:
            ...
            do {
                ...
                /// 重新计算秒数组以及索引
                contentView.calculateSecondList()
                contentView.reloadComponent(5)
                contentView.secondIndex = contentView.secondList.firstIndex(of: PickerDateModel(id: secondString, name: "")) ?? 0
                contentView.selectRow(contentView.secondIndex, inComponent: 5, animated: false)
            }
        case 1:
            let dayString = contentView.dayList[contentView.dayIndex].id
            let hoursString = contentView.hoursList[contentView.hoursIndex].id
    ...
    func pickerContentView(_ contentView: PickerViewController) -> String {
        return "\(contentView.yearList[contentView.yearIndex].id)-\(contentView.monthList[contentView.monthIndex].id)-\(contentView.dayList[contentView.dayIndex].id) \(contentView.hoursList[contentView.hoursIndex].id):\(contentView.minuteList[contentView.minuteIndex].id):\(contentView.secondList[contentView.secondIndex].id)"
    }
}
